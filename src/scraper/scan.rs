...
pub fn scan_gdi_live() -> Vec<DisplayRow> {
    ...
    unsafe {
        for i in 0..64 {
            ...
            let device_name = String::from_utf16_lossy(&device.DeviceName).trim_matches('\0').to_string();
            ...
            if EnumDisplayDevicesW(PCWSTR::from_raw(device.DeviceName.as_ptr()), 0, &mut monitor_device, 0).as_bool() {
                hw_id_path = String::from_utf16_lossy(&monitor_device.DeviceID).trim_matches('\0').to_string();
            }
...
fn parse_edid(bytes: &[u8]) -> (String, String, String) {
    ...
    for offset in [54, 72, 90, 108] {
        if offset + 18 <= bytes.len() && &bytes[offset..offset+4] == b"\x00\x00\x00\xff" {
            sn = String::from_utf8_lossy(&bytes[offset+4..offset+18]).trim().to_string();
        }
    }
    let (w_mm, h_mm) = ((bytes[21] as u32) * 10, (bytes[22] as u32) * 10);
    let (w_px, h_px) = (
        ((bytes[58] as u32 & 0xF0) << 4) | (bytes[56] as u32),
        ((bytes[61] as u32 & 0xF0) << 4) | (bytes[59] as u32)
    );
...
fn get_dpi_scaling_from_name(_name: &str) -> String {
    ...
    unsafe { let _ = EnumDisplayMonitors(None, None, Some(monitor_enum_proc), LPARAM(&mut dpi_str as *mut String as isize)); }
    dpi_str
}
